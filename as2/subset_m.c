...
void best_subset_m(int length, int all_jobs[][2],int num_workers){
  ...
  int result = print_solution_m(0,0,0,length,all_jobs,accpeted_array,0);
  ...
  int completion_time = get_completion_time_m(num_accepted_jobs, accpeted_array);
...
int find_best_m(int t,int j, int v,int length,int all_jobs[][2]){
  ...
  int choose = find_best_m(t+1,j+all_jobs[t][0],v+j+all_jobs[t][0],length,all_jobs);

  int not_choose = find_best_m(t+1,j,v+all_jobs[t][1],length,all_jobs);
...
int print_solution_m(int t,int j, int v,int length,int all_jobs[][2],int accpeted_array[][2],int x){
  if (t != length-1){
    int result = find_best_m(t,j,v,length,all_jobs);
    if (find_best_m(t+1,j,v+all_jobs[t][1],length,all_jobs) == result){
      print_solution_m(t+1, j, v+all_jobs[t][1], length, all_jobs, accpeted_array,x);
    }
    else{
      accpeted_array[x][0] = all_jobs[t][0];
      accpeted_array[x][1] = all_jobs[t][1];
      x += 1;
      print_solution_m(t+1,j+all_jobs[t][0],v+j+all_jobs[t][0],length,all_jobs,accpeted_array,x);
    }
    return result;
  }
  return 0;
