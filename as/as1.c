/* purposes: assignment1
 * Authors: Jinglong Zhao
 * Reference: pervious lab
 * Dates: Started on October 10, 2020
...
int scan_jobs(int length,int jobs[length][2]){
...
void print_instance(int length,int jobs[length][2]){
  //print a given list
  for (int i=0;i<length;i++){
    for (int j=0;j<2;j++){
      printf("%d ", jobs[i][j]);
...
void sort(int length,int list[length][2]){
...
int caculate(int length,int list[length][2],int index){
